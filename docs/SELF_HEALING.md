...
2. **自调试**：Codex 读取结构化事故、近期脱敏日志、Git revision 和同指纹历史，默认以 `model_reasoning_effort="minimal"`、`service_tier="fast"` 和 fast mode 在只读沙箱中分析根因。
...
- Codex 以 `-a never`、`--ephemeral` 和 `--ignore-user-config` 运行，并显式设置 `model_reasoning_effort="minimal"`、`service_tier="fast"` 与 `features.fast_mode=true`；不允许交互批准、联网搜索或额外可写目录。
