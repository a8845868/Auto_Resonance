"""Enter an existing session once through ReadOnlyActionGuard and observe HOME."""
...
def run(
    ...
    *,
    ...
    with installed_read_only_guard(guard):
        result = SessionEntryResolver(
            frame_provider=screenshot,
            tap=guarded_entry,
            timeout=timeout,
            max_attempts=max(3, int(timeout)),
            poll_interval=1.0,
...
def main() -> int:
    ...
    output.mkdir(parents=True, exist_ok=True)
    (output / "SESSION_ENTRY_RESULT.json").write_text(
